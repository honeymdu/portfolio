...
const DESIGNS: Record<Tab, DesignData> = {
  translator: {
    ...
    flow: [
      ...
      [
        { icon: MessageSquare, label: "TranslationService", sub: "Google Cloud Translation API v2 · Mono<String> reactive", color: "purple" },
    ...
    components: [
      ...
      { icon: Mic, name: "Google Cloud STT + TTS", role: "STT: streaming recognition with one session per call leg (agent + customer independently). TTS: PCM output encoded to match Vonage WebSocket format. Voice selection stored per-agent in AgentPreference. Microsoft Cognitive Services included as fallback STT provider.", color: "purple" },
      { icon: Globe, name: "Google Cloud Translation API", role: "Core translation layer called via TranslationService. Uses Cloud Translation API v2 (Neural Machine Translation) for text segments produced by STT. Called reactively via Mono<String> so the audio thread is never blocked during API round-trips. Supports 4 language pairs in production (EN ↔ FR, ES, AR, ZH). Domain-specific accuracy tuned to airline call-center vocabulary — achieves ~90% accuracy vs. ~70% from generic models. API key scoped per Cloud project; results are not cached (live utterances must always be fresh).", color: "indigo" },
    ...
    nfrs: [
      ...
      { metric: "Translation Engine", target: "Google Cloud Translation API v2 (NMT)" },
